...
export function TaskModal({ board, taskId, onClose }: Props) {
  ...
  const patch = (p: Partial<Task>) => updateTask(board.id, task.id, p);
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label="Group">
            <select
              value={task.groupId}
              onChange={(e) => moveTask(board.id, task.id, e.target.value)}
              className="w-full rounded-md border px-2 py-1.5 text-sm focus:border-brand-500 focus:outline-none"
            >
              {board.groups.map((g) => (
        ...
        <Field label="Description">
          <textarea
            ...
            className="w-full resize-y rounded-md border px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100"
          />
        </Field>
      </div>
    </Modal>
